...
bool Physical_location::check_room_no(string val)
{
    ...
    int vsize=val.size();
    if(vsize != Metadata::get_room_no_size()) return false;
    else
    {
        for(int i=0;i<vsize;i++) //checks if every character is a digit or not
        {
...
bool Physical_location::check_build_no(string val)
{
    //checks if the size of string is okay
    int vsize=val.size();
    if(vsize != Metadata::get_building_no_size()) return false;
    else
    {
        for(int i=0;i<vsize;i++) //checks if every character is a digit or not
        {
